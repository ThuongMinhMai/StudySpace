...
interface FilterComponentProps {
  ...
  onClearFilters: () => void // Add the prop to handle clearing filters
}
...
const FilterComponent: React.FC<FilterComponentProps> = ({ onFilterChange, onClearFilters }) => {
  ...
  const [ratingSort, setRatingSort] = useState<'highest' | 'lowest' | 'all'>('all')
  ...
  const handleApplyFilters = () => {
    const filters = {
      ...
      ratingSort: ratingSort === 'all' ? 'all' : ratingSort,
      priceRange,
      selectedUtilities: selectedUtilities.length === 0 ? 'all' : selectedUtilities,
    }
  ...
  const handleClearFilters = () => {
    ...
    // Trigger the parent function to clear filters
    onClearFilters()
  }
  ...
  return (
    ...
    >
      <div className="filter-container w-4/5 flex flex-col gap-5 text-lg">
        ...
        <div className="filter-section">
          <p>Sort by Rating:</p>
          <Radio.Group value={ratingSort} onChange={(e) => setRatingSort(e.target.value)}>
            <Radio value="all">All</Radio>
            <Radio value="highest">Highest Rating</Radio>
            <Radio value="lowest">Lowest Rating</Radio>
          </Radio.Group>
        </div>
        ...
        <div className="filter-section">
          ...
          />
          {selectedUtilities.length === 0 && <p className='text-base mt-2 text-gray-500'>*All utilities selected by default.</p>}
        </div>
